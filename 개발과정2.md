...
Date끼리 연산할 때는 결과가 Millisecond 단위로 리턴된다. 그리고 대부분의 경우 이를 일(day) 수와 같이 원하는 형태로 바꾸기 위한 계산을 반드시 거치기 때문에 부동소수점 형태를 제대로 계산하지 못하는 parseInt()는 적절치 않다. 게다가 문자를 정수로 변환해주는 parseInt()의 본래의 목적과도 어울리지 않는다. (구글링을 하다보면 이런식으로 계산하고 있는 경우를 쉽게 볼 수 있다. )
